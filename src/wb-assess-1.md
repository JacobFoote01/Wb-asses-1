...
```
console.log is a function used to log an argument to the console. return is used to end the function.
```

**_What's the difference between defining and calling a function?_**

```
Defining a function is just defining an aspect of the code. Calling a function is how you run the function.
```
...
```
it is a number. Here is an example(1, 2, 3)
```
...
```
Arrays are used to store many values inside of a single variable using [].
```
